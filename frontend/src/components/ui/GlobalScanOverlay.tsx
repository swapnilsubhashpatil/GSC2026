...
export function GlobalScanOverlay() {
  ...
  useEffect(() => {
    if (!isLoading) {
      // Delay reset to allow 100% completion animation
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-bg-primary/95 backdrop-blur-xl flex items-center justify-center animate-fade-in">
      <div className="w-full max-w-lg px-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">Global Scan</h2>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-mono text-indigo-400">{message}</span>
        ...
        <div className="rounded-xl border border-white/5 bg-bg-elevated p-4 h-48 overflow-hidden">
          <div className="flex items-center gap-2 mb-3">
            <Terminal className="w-3.5 h-3.5 text-gray-600" />
            <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">System Log</span>
          </div>
          <div className="space-y-1.5">
            {logs.map((log, i) => (
              <div key={i} className="flex items-center gap-2 text-[11px] text-gray-400 font-mono animate-fade-in">
                <span className="text-gray-700">[{new Date().toLocaleTimeString()}]</span>
                <span className="text-emerald-500/60">✓</span>
                {log}
              </div>
            ))}
            {isLoading && <div className="flex items-center gap-2 text-[11px] text-gray-600 font-mono"><span className="animate-pulse">_</span></div>}
          </div>
        </div>
      </div>
    </div>
