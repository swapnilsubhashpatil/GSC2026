...
export function RiskBadge({ score, showLabel = true, size = 'md' }: RiskBadgeProps) {
  ...
  return (
    <div className={`inline-flex items-center rounded-full border font-mono font-bold ${riskBg(score)} ${sizes[size]}`}>
      <span className={riskColor(score)}>{score}</span>
      {showLabel && <span className="text-gray-500 ml-1 text-[10px]">{score >= 70 ? 'CRIT' : score >= 40 ? 'ELEV' : 'LOW'}</span>}
    </div>
  );
}
